...
# and plot the difference between the analytical and actual rotation curves for each snapshot. This should
...
# Snapshots: 0, 65, 190, 300, 420, 475, 525, 580, 625
...
def rotationCurve(snap, rj):
    ...
    rr = np.arange(0.001, rj, 0.5) # From 0.001 to rj, spaced by 0.5
...
# Store Jacobi Radii
jacobiRadii = np.array([rj1,rj2,rj3,rj4,rj6,rj7,rj8,rj9,rj10])

snaps = np.array([0, 65, 190, 300, 420, 475, 525, 580, 625])


# Create an array to hold positions + rotation curves to loop through later
RR_RCs = [rotationCurve(snaps[i], jacobiRadii[i]) for i in range(9)]

# create array for time of each snapshot
sToT = 10/0.7/1000 # so T is in Gyrs
times = np.array([0, 65 * sToT, 190 * sToT, 300 * sToT, 420 * sToT, 475 * sToT,
...
rotatedRs = np.array([rn1,rn2,rn3,rn4,rn6,rn7,rn8,rn9,rn10])
rotatedVs = np.array([vn1,vn2,vn3,vn4,vn6,vn7,vn8,vn9,vn10])
...
rhos = np.array([rho1,rho2,rho3,rho4,rho6,rho7,rho8,rho9,rho10])
...
vphis = np.array([vphi1,vphi2,vphi3,vphi4,vphi6,vphi7,vphi8,vphi9,vphi10])
    
# Determine the mean vphi per radius

# Initialize Array for Radius = corresponding Jacobi Radius
Rs = [np.arange(0, jacobiRadii[i], 0.5) for i in range(9)]

# Initialize Empty Array for Velocity 
# (same size as radial array)

#Vels = [np.zeros(len(Rs[i])) for i in range(10)]

# compute the mean vphi in radial bins

lens = [len(Rs[i]) for i in range(9)]
maxR = max(lens)

Vs = [np.zeros(lens[i]) for i in range(9)]

#print(len(rhos[0]))

for i in range(9):
    for a in range(len(Rs[i])):
        #print(rhos[i])
        index = np.where((rhos[i] > 0.5*a) & (rhos[i] < 0.5*a+0.5)) # walking out in radial bins
        #print(index)
        #print(len(rhos[i][index]))
        Vs[i][a] = np.mean(np.abs(vphis[i][index])) # mean velocity
...
for i in range(9):
    ax[0].plot(RR_RCs[i][0], RR_RCs[i][1], label = f"Time = {times[i]:.2f} (Gyrs)")


# Add axis labels and title
ax[0].set_title("M33's Analytical Rotation Curve Evolution", fontsize = 30)
ax[0].set_xlabel('Radius (kpc)', fontsize=22)
ax[0].set_ylabel('Circular Velocity (km/s) ', fontsize=22)
...
for i in range(9):
    #print(Vels[i])
    if (i<1):
        ax[1].plot(Rs[i], Vs[i], linewidth = 4, label = f"Time = {times[i]:.2f} (Gyrs)")
    else:
        ax[1].plot(Rs[i], Vs[i], label = f"Time = {times[i]:.2f} (Gyrs)")
        
# Add axis labels and title
ax[1].set_title("M33's Mean Circular Velocity Evolution", fontsize = 30)
ax[1].set_xlabel('Radius (kpc)', fontsize=22)
ax[1].set_ylabel('Circular Velocity (km/s) ', fontsize=22)
...
# To make quantitative comparison, one possiblity is subtracting each vphi from the 
# analytical circular velocity, or from the initial analytical/actual vphi
...
for i in range(9):
    #print(Vels[i])
    if (i<1):
        plt.plot(Rs[i], RR_RCs[i][1] - Vs[i], linewidth = 4, label = f"Time = {times[i]:.2f} (Gyrs)")
    else:
        plt.plot(Rs[i], RR_RCs[i][1] - Vs[i], label = f"Time = {times[i]:.2f} (Gyrs)")


plt.vlines(3, 0, 80, linestyles="dotted", colors="k")
plt.vlines(12, 0, 80, linestyles="dotted", colors="k")
# Add axis labels and title
plt.title(r"M33's Analytical - Mean v$_\phi$ Rotation Curve Evolution", fontsize = 30)
plt.xlabel('R (kpc)', fontsize=22)
plt.ylabel(r'Analytical v$_\phi$ - Calculated v$_\phi$ (km/s)', fontsize=22)
