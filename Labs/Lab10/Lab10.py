...
class M33AnalyticOrbit:
    ...
    def __init__(self, filename):
        ...
        self.Vc = 180 # circular speed of M31 halo in km/s (Hmwk 5)
    ...
    def DynamicalFriction(self, Msat, Vc, r, v):
        """
        Method that returns the deceleration from dynamical friction felt by M33
        owing to its motion through the dark matter halo of M31

        Parameters
        ----------
        Msat : float
            Mass of the satellite in Msun.
        Vc : float
            Maximum circular velocity of the host dark matter halo (km/s).
        r : np.ndarray of floats
            Position vector of the satellite relative to the host (kpc).
        v : np.ndarray of floats
            Velocity vector of the satellite relative to the host (km/s).

        Returns
        -------
        DF : np.ndarray of floats.
            Deceleration vector owing to dynamical friction
        """
        # magnitude of the satellite position vector
        r_mag = np.sqrt(r[0]**2 + r[1]**2 + r[2]**2) 
        
        # magnitude of the satellite velocity vector
        v_mag = np.sqrt(v[0]**2 + v[1]**2 + v[2]**2) 
        
        # For the Coulomb Logorithm
    ...
    def M31Accel(self, r, v):
        ...
        ----------
        r : `np.ndarray`
            Position vector to find the acceleration at (kpc)
        v : np.ndarray
            Velocity vector for the DF calculation (km/s)
